...
func workerEncrypt(key, iv []byte, inputChan <-chan block, outputChan chan<- block, wg *sync.WaitGroup) {
	defer wg.Done()
	for blk := range inputChan {
		...
		// Генерируем уникальный IV для каждого блока на основе индекса
